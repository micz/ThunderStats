...
miczThunderStatsTab.ui={
	...
	draw7DaysGraph3_ArrangeData:function(data_array,w,tot_days,do_double){
		...
		let _data_handles;
		if(do_double){
			_data_handles=['sent','rcvd'];
			data_input=data_array;
		}else{
			_data_handles=['sent'];
			data_input['sent']=data_array;
		}
		...
		for(let h_el in _data_handles){
			...
			current_data.data.sort(this.draw7DaysGraph3_ArrayCompare);
			...
			current_data.data_norm.sort(this.draw7DaysGraph3_ArrayCompare);
	...
	draw7DaysGraph3_ArrayCompare:function(a,b){
		if(a.day < b.day){
			return -1;
		}
		if(a.day > b.day){
